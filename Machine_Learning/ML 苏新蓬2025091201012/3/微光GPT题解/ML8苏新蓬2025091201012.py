...
# 读取莎士比亚文本
with open('D:/My_CODE/Git_ML/when-I-learn-ML/微光/ml-8 transformers/input.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
for iter in range(max_iters):
    ...
    t2=time.time()
    t=t2-t1
    print(iter ,'次迭代',t,'loss=',loss.item())
    # 6. 定期评估
    if iter % eval_interval == 0:
        with torch.no_grad():
            val_logits, val_loss = model(*get_batch('val'))  #*解包元组
